...
def render_precommit_script(archive: ProjectArchive) -> str:
    ...
    lines = [
        ...
        "if ((os.environ.get(\"WORKTREES_ENABLED\",\"0\") or \"0\").strip().lower() not in {\"1\",\"true\",\"t\",\"yes\",\"y\"}) and ((os.environ.get(\"GIT_IDENTITY_ENABLED\",\"0\") or \"0\").strip().lower() not in {\"1\",\"true\",\"t\",\"yes\",\"y\"}):",
        "    sys.exit(0)",
        "",
        ...
        "try:",
        "    argv = [\"uv\",\"run\",\"python\",\"-m\",\"mcp_agent_mail.cli\",\"guard\",\"check\",\"--stdin-nul\",\"--repo\", root]",
        "    if ADVISORY:",
        "        argv.append(\"--advisory\")",
        "    proc = subprocess.run(argv, input=payload, check=False)",
        "    rc = proc.returncode",
        "    if rc != 0:",
        "        if ADVISORY:",
        ...
        "    sys.exit(0)",
        "except Exception:",
        "    # Fallback: try system Python directly",
        "    try:",
        "        argv = [\"python\",\"-m\",\"mcp_agent_mail.cli\",\"guard\",\"check\",\"--stdin-nul\",\"--repo\", root]",
        "        if ADVISORY:",
        "            argv.append(\"--advisory\")",
        "        proc = subprocess.run(argv, input=payload, check=False)",
        "        rc = proc.returncode",
